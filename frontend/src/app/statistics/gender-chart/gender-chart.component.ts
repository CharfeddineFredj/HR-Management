...
export class GenderChartComponent implements OnInit {
  ...
  public chartLabels: string[] = ['Male', 'Female'];
  ...
  loadChartData(): void {
    this.employeeService.getEmployeeCountByGender().subscribe({
      next: data => {
        this.ngZone.run(() => {
          this.chartData.datasets[0].data = [data['male'] || 0, data['female'] || 0];
  ...
  downloadChart(format: string): void {
    const chart = document.getElementById('genderChart') as HTMLCanvasElement;
    if (chart) {
      if (format === 'png') {
        ...
        link.download = 'gender_chart.svg';
        link.click();
      }
    }
  }
  ...
  downloadCSV(): void {
    ...
    link.download = 'gender_chart.csv';
    link.click();
  }
