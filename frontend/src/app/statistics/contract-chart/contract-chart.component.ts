...
export class ContractChartComponent implements OnInit {
  ...
  public chartLabels: string[] = ['CDI', 'CDD', 'CIVP', 'Karama'];
    ...
    datasets: [
      {
        ...
        hoverBackgroundColor: ['#36A2EB90', '#FF638490', '#FFCE5690', '#4BC0C090'],
        borderWidth: 1
      }
    ]
  };
  ...
  loadChartData(): void {
    this.employeeService.getEmployeeCountByContractType().subscribe({
      next: data => {
        this.ngZone.run(() => {
  ...
  downloadChart(format: string): void {
    const chart = document.getElementById('contractChart') as HTMLCanvasElement;
    if (chart) {
      if (format === 'png') {
        const link = document.createElement('a');
        link.href = chart.toDataURL('image/png');
        link.download = 'contract_chart.png';
        link.click();
      } else if (format === 'svg') {
        const svg = this.convertToSVG(chart);
        const link = document.createElement('a');
        link.href = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
        link.download = 'contract_chart.svg';
        link.click();
      }
    }
  }
  ...
  downloadCSV(): void {
    ...
    link.download = 'contract_chart.csv';
    link.click();
  }
